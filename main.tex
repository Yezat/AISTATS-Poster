...
\begin{itemize}
    \item Generalisation Error: \(\generr = \EEb{y,\x}{ \mathbb{1}(y \neq \hat{y} (\what, \x)) } \)
    \item Adversarial Generalisation Error: \(\advgenerr = \EEb{y,\x}{
        \max_{\norm{\vecdelta}_{\Sigmaupsilon^{-1}} \leq \advgencost} \mathbb{1}(y \neq \hat{y} (\what, \x + \vecdelta))
    } \)
    \item Boundary Error: \(\advgenerr = \generr + \bounderr\) where \(\bounderr\) are the attackable samples.
    \item Usefulness and Robustness:\begin{align}
        \usefulmetric_{\wstar}
...
\postercaption{Optimal defense strategy depends on feature geometry}
\end{center}
